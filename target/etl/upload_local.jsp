...
<body>
	...
	<div class="container-fluid">
		<div class="row-fluid">
			...
			<div class="span9">
				<div class="legend">Gestion des fichiers</div>
				<br>
				<form id="fileupload" method="POST" action="uploadform"
					enctype="multipart/form-data">
					<div class="row fileupload-buttonbar span10">
						<div class="span7">
							<span class="btn btn-success fileinput-button"> <i
								class="icon-plus icon-white"></i> <span>Ajouter</span> <input
				...
				<table class="table table-striped">
					<tbody class="files" id="total-size">
					</tbody>
				</table>
				<br>
				<div class="well">
...
{% for (var i=0, file; file=o.files[i]; i++) { %}
    <tr class="template-download fade">
        ...
        {% } else { %}
            ...
            {% } %}</td>
            <td class="name">
                <a href="{%=file.url%}" target="_blank" title="{%=file.name%}" rel="{%=file.thumbnail_url&&'gallery'%}" download="{%=file.name%}"><i class="icon-download-alt"></i> {%=file.name%}</a>
            </td>
            <td class="size"><span>{%=o.formatFileSize(file.size)%}</span></td>
            <td colspan="2"></td>
        {% } %}
